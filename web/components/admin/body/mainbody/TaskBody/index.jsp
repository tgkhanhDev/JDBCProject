...
<html>
    ...
    <body>
        ...
        <!--form update--> 
        <!--layer-->
        <%                Account managerAcc = (Account) request.getAttribute("formAccount");
        %>
        <div id="formUpdate" class="transition-all ease-in-out <%=  ((managerAcc == null)? "hidden": "")  %>">
            ...
            <div class="bg-[#f6f6f6] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 px-20 py-10 z-10">
                ...
                <div class="">Bạn có chắc muốn giao việc cho: </div>
                <%
                    if (managerAcc != null)
                    {
                %>
                <div><%=  (managerAcc.getFirstName() + " " + managerAcc.getLastName())%></div>
                <%
                    }
                %>
            </div>
